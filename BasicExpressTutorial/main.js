...
app.use(express.urlencoded({extended: false})) /*use piece of middleware globally
...
function getWeather(req, res, next){ //middleware, when done next is called
...
app.get('/', getWeather, (req, res) => { // localhost:3000
...
app.post('/result', (req, res) => {
    if(req.body.color.trim().toUpperCase() === 'BLUE'){ //can do req.body.color thanks to the urlencoded thing at top
...
app.get('/api/pets', (req, res) =>{
    res.json([{name: 'Meowsalot', species: 'cat'},{name: 'Barksalot', species: 'dog'}])
});
